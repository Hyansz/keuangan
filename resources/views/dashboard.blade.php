...
@section('tambah')
                ...
                <div class="mb-3 pl-1">
                    ...
                    <select name="kategori_id" id="kategori_id" required>
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" data-jenis="{{ $category->type }}">{{ $category->name }}</option>
                        @endforeach
                    </select> 
                </div>
                <div class="mb-3">
    ...
    <script>
        function updateKategori() {
            const tipeDipilih = document.querySelector('input[name="tipe"]:checked').value;
            const selectKategori = document.getElementById('kategori_id');
            const semuaOption = selectKategori.querySelectorAll('option');

            semuaOption.forEach(option => {
                if (option.getAttribute('data-jenis') === tipeDipilih) {
            ...
            // Reset ke pilihan pertama yang cocok
            const optionPertama = Array.from(semuaOption).find(opt => opt.getAttribute('data-jenis') === tipeDipilih);
            if (optionPertama) {
                selectKategori.value = optionPertama.value;
            }
        }
